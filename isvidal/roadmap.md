...
- **Phase 5b — ✅ DONE (2026-04-09).** External 3-subagent review (methodology / code / output validation) found 14 issues. All critical fixes applied: null guards in `filter_isvidal.py` for `fecha_iso`/`timestamp`, empty-df guards in `analyze_isvidal.py:main` and `app.py`, Streamlit cache invalidation via `(path, mtime)` tuple key (no more stale data after re-running pipeline), single write of `opinion_excerpts.parquet` (removes risk of stance-less stale parquet), `build_report` now receives `excerpts_with_stance`. Analysis correctness: dedup overlapping excerpts in `build_opinion_excerpts` (removes 14 double-counted duplicates), `shadcdn → shadcn` alias (typo merge, score 3.47), removed `DX` from dict (it's a concept, not a tool). Polars perf: replaced `map_elements` with native expressions for year_weight and verdict computation. Verdict logic: reordered so `mixto` is checked first → Zustand correctly demoted from `recomienda` to `mixto` (4 pos / 2 neg, recent 2025 negatives "olvidate de zustand"). New mixto callout in dashboard between bar charts and table.
- **Phase 5c — ✅ DONE (2026-04-09).** Final 3-subagent review round (fix verifier + output validator + fresh code review). Scores 9/10, 9/10, 7/10. Fix verifier: all 14 Phase 5b fixes verified correctly applied, no regressions. Output validator: 100% ground-truth alignment on all 4 key posts (#1194/#1198/#1196/#1128), 0 dedup duplicates, 8/8 stance sample correct, all 5 critical term sanity checks PASS. Code reviewer found 1 critical + 3 warnings: (1) `apply_stance_cache` crashed on malformed `stance_cache.json` — wrapped in try/except with fallback to empty dict. (2) `app.py:472` `json.loads(code_blocks)` had no error handler — added try/except. (3) Unused `timezone` import in `filter_isvidal.py` — removed. (4) Slider crashed with `earliest_year == latest_year` (single-year corpus edge case) — added `slider_min = earliest_year if earliest_year < latest_year else latest_year - 1` guard. Also corrected misleading dedup docstring in `build_opinion_excerpts`. Pipeline verified end-to-end: Zustand mixto, shadcn merged, DX removed, 186 excerpts with 0 duplicates, dashboard loads with 0 errors.
