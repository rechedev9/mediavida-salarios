...
Datos: 231 programadores, Oct 2017 – Abr 2026, salario bruto anual reportado.
"""
...
# Salary range
sal_min = int(df_all["salario_bruto_anual"].min())
sal_max = int(df_all["salario_bruto_anual"].max())
...
df = df.filter(
    pl.col("salario_bruto_anual").is_between(sal_range[0], sal_range[1])
...
st.caption(
    ...
    f"· {len(df_all)} respuestas válidas · Oct 2017 – Abr 2026 · Salario bruto anual reportado"
)
...
st.subheader("Distribución de salario bruto anual")

fig_hist = px.histogram(
    df.to_pandas(),
    x="salario_bruto_anual",
    nbins=30,
    labels={"salario_bruto_anual": "Salario bruto anual €"},
...
if top_cities:
    ...
    city_order = (
        ...
        .group_by("ciudad")
        .agg(pl.col("salario_bruto_anual").median().alias("med"))
...
by_year = (
    df.group_by("year")
    .agg([
        pl.col("salario_bruto_anual").median().round(0).alias("Mediana bruto anual €"),
        pl.len().alias("n"),
    ])
    .filter(pl.col("n") >= 2)
    .sort("year")
...
if len(by_year) > 1:
    fig_time = px.line(
        by_year,
        x="year",
        y="Mediana bruto anual €",
        labels={"year": "Año del post", "Mediana bruto anual €": "Salario bruto anual €"},
        markers=True,
    )
    ...
    for _, row in by_year.iterrows():
        fig_time.add_annotation(
            x=row["year"], y=row["Mediana bruto anual €"],
...
table_cols = [
    "autor", "year", "salario_bruto_anual",
    "ciudad", "modalidad", "pais", "experiencia_anos", "tecnologia",
]
df_table = (
    df.select(table_cols)
    .sort("salario_bruto_anual", descending=True)
    .with_columns(
...
- Los salarios se muestran tal cual fueron reportados, sin ajuste por inflación
""")
